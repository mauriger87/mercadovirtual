...
function activarSeccion(key) {
    ...
    if (typeof gtag === 'function') {
        gtag('event', 'seleccionar_seccion', { seccion: key });
    }
    actualizarFiltros();
    renderProductos();
...
// ============================================
const SECCION_LABEL = {
    bebidas: 'Bebidas', almacen: 'Almacén', perfumeria: 'Perfumería', limpieza: 'Limpieza'
};
...
function renderMasPedidos() {
    ...
    if (seccionActiva === 'ofertas' || filtros.busqueda.trim() || filtros.familia) {
    ...
    if (seccionActiva === 'todos') {
        ...
        const enOferta = productos.filter(p =>
            p.stock > (config.stockCritico || 0) &&
            ((p.ean && getItemFolleto(p.ean)) || ofertasHoy.some(o => o.productoId === p.id))
        );
        if (enOferta.length) {
            top = enOferta.sort((a, b) => (b.venta_diaria || 0) - (a.venta_diaria || 0)).slice(0, 10);
            titulo = '🔥 Ofertas destacadas';
        } else {
            top = productos.filter(p => p.stock > (config.stockCritico || 0))
                .sort((a, b) => (b.venta_diaria || 0) - (a.venta_diaria || 0)).slice(0, 10);
            titulo = '⭐ Lo que más pedís';
        }
    } else {
        const sm = SECCIONES_MENU[seccionActiva];
        if (!sm || !sm.seccion) { cont.style.display = 'none'; cont.innerHTML = ''; return; }
        let base = productos.filter(p => p.stock > (config.stockCritico || 0) && p.seccion === sm.seccion);
        if (sm.familias) base = base.filter(p => sm.familias.includes(p.familia));
        top = base.sort((a, b) => (b.venta_diaria || 0) - (a.venta_diaria || 0)).slice(0, 10);
        titulo = `⭐ Lo más pedido en ${SECCION_LABEL[seccionActiva] || 'esta sección'}`;
    }
    ...
    cont.innerHTML = `
        <div class="mas-pedidos-titulo">${escapeHTML(titulo)}</div>
...
function renderFamiliaChips() {
    ...
    cont.querySelectorAll('.familia-chip').forEach(btn => {
        btn.addEventListener('click', () => {
            ...
            productosVisibles = PRODUCTOS_POR_PAGINA;
            if (typeof gtag === 'function' && btn.dataset.familia) {
                gtag('event', 'seleccionar_familia', { seccion: seccionActiva, familia: btn.dataset.familia });
            }
            renderProductos();
        });
    });
