...
function generarWhatsApp() {
    ...
    } else if (typeof ENVIO_AUTOMATICO_ACTIVO !== 'undefined' && !ENVIO_AUTOMATICO_ACTIVO) {
...
async function registrarPedido(pedido) {
    try {
        const payload = {
            ...
            eans:  pedido.items.map(i => i.ean || '-').join(' | '),
            total: pedido.total,
            envio: pedido.envio,
        ...
        if (data && data.ok) {
            ...
            console.error('❌ El Apps Script rechazó el pedido:', data);
        }
    } catch (err) {
